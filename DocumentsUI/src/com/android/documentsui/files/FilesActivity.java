...
public class FilesActivity extends BaseActivity implements AbstractActionHandler.CommonAddons {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.option_menu_create_dir) {
            ...
            Log.d(TAG, "Called menu_create_dir");
        } else if (id == R.id.option_menu_new_window) {
            mInjector.actions.openInNewWindow(mState.stack);
            Log.d(TAG, "Called menu_new_window");
        } else if (id == R.id.option_menu_settings) {
            mInjector.actions.openSettings(getCurrentRoot());
            Log.d(TAG, "Called menu_settings");
        } else if (id == R.id.option_menu_select_all) {
            Log.d(TAG, "Called menu_select_all");
            mInjector.actions.selectAllFiles();
        } else if (id == R.id.option_menu_inspect) {
            // Getting the Path of the file and changing it to the absolute path
            //TODO
            String relativePath = getCurrentDirectory().documentId;
            Log.d(TAG, "documentId IS:" + relativePath);
            String absolutePath = relativePath.replace("primary:","/storage/emulated/0/");

            // Reading the lines that are already marked as to delete
            File file = new File("/storage/emulated/0/Pictures/test.txt");
            ...
            // Check if file is already listed as to delete
            if (lines.contains(absolutePath)){
                lines.remove(absolutePath);
            }
            else{
                lines.add(absolutePath);
            }

            // Write the updates lines back into the marked_to_delete file. (Technically its overwriting the whole file but fits our purpose enough for now.)
            try (BufferedWriter writer = new BufferedWriter(new FileWriter("/storage/emulated/0/Pictures/test.txt", false))){
                for (String line: lines){
                    writer.write(line);
                    writer.newLine();
                }
            } catch (IOException e){
                android.util.Log.d(TAG, "onOptionsItemSelected:" + e);
            }
